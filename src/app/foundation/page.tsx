...
export default function FoundationStatusPage() {
  ...
  const filtered =
    filter === "all"
      ? foundationComponents
      : foundationComponents.filter((c) => c.status === filter);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-warm-200 text-sm">
          Real-time progress on all 16 UBC components. Last updated: March 2026
        </p>
      </div>
...
function ComponentCard({
  ...
  return (
    <button
      onClick={onToggle}
      className="text-left bg-warm-900/30 border border-warm-800 hover:border-civic-blue rounded-lg p-4 transition-colors w-full"
    >
      ...
      <span className="text-xs text-warm-200">{component.progressPct}%</span>
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-warm-800">
          ...
          <div className="mt-3">
            <span className="text-xs text-civic-blue-light cursor-pointer hover:underline">
